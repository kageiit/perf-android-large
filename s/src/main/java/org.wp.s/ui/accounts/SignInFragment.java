...
import org.wp.s.BuildConfig;
import org.wp.s.R;
import org.wp.s.WordPress;
import org.wp.s.models.Blog;
import org.wp.s.ui.accounts.helpers.FetchBlogListAbstract.Callback;
import org.wp.s.ui.accounts.helpers.FetchBlogListWPCom;
import org.wp.s.ui.accounts.helpers.FetchBlogListWPOrg;
import org.wp.s.ui.accounts.helpers.LoginAbstract;
import org.wp.s.ui.accounts.helpers.LoginWPCom;
...
import org.wp.s.util.HelpshiftHelper.Tag;
import org.wp.s.util.JSONUtils;
import org.wp.s.util.NetworkUtils;
import org.wp.s.util.StringUtils;
import org.wp.s.util.ToastUtils;
import org.wp.s.util.WPUrlUtils;
import org.wp.s.widgets.WPTextView;
...
//            track(Stat.LOGIN_FAILED, null);
    ...
    protected void showInvalidUsernameOrPasswordDialog() {
        ...
        nuxAlert = SignInDialogFragment.newInstance(getString(org.wp.s.R.string.nux_cannot_log_in),
                getString(org.wp.s.R.string.username_or_password_incorrect),
                org.wp.s.R.drawable.noticon_alert_big, 3, getString(
                        org.wp.s.R.string.cancel), getString(
                        org.wp.s.R.string.forgot_password), getString(
                        org.wp.s.R.string.contact_us), SignInDialogFragment.ACTION_OPEN_URL,
    ...
    protected void signInError(int messageId, String clientResponse) {
        FragmentTransaction ft = getFragmentManager().beginTransaction();
        SignInDialogFragment nuxAlert;
        if (messageId == org.wp.s.R.string.username_or_password_incorrect) {
            ...
            return;
        } else if (messageId == org.wp.s.R.string.invalid_site_url_message) {
