...
import org.wp.g.BuildConfig;
import org.wp.g.R;
import org.wp.g.WordPress;
import org.wp.g.models.Blog;
import org.wp.g.ui.accounts.helpers.FetchBlogListAbstract.Callback;
import org.wp.g.ui.accounts.helpers.FetchBlogListWPCom;
import org.wp.g.ui.accounts.helpers.FetchBlogListWPOrg;
import org.wp.g.ui.accounts.helpers.LoginAbstract;
import org.wp.g.ui.accounts.helpers.LoginWPCom;
...
import org.wp.g.util.HelpshiftHelper.Tag;
import org.wp.g.util.JSONUtils;
import org.wp.g.util.NetworkUtils;
import org.wp.g.util.StringUtils;
import org.wp.g.util.ToastUtils;
import org.wp.g.util.WPUrlUtils;
import org.wp.g.widgets.WPTextView;
...
//            track(Stat.LOGIN_FAILED, null);
    ...
    protected void showInvalidUsernameOrPasswordDialog() {
        ...
        nuxAlert = SignInDialogFragment.newInstance(getString(org.wp.g.R.string.nux_cannot_log_in),
                getString(org.wp.g.R.string.username_or_password_incorrect),
                org.wp.g.R.drawable.noticon_alert_big, 3, getString(
                        org.wp.g.R.string.cancel), getString(
                        org.wp.g.R.string.forgot_password), getString(
                        org.wp.g.R.string.contact_us), SignInDialogFragment.ACTION_OPEN_URL,
    ...
    protected void signInError(int messageId, String clientResponse) {
        FragmentTransaction ft = getFragmentManager().beginTransaction();
        SignInDialogFragment nuxAlert;
        if (messageId == org.wp.g.R.string.username_or_password_incorrect) {
            ...
            return;
        } else if (messageId == org.wp.g.R.string.invalid_site_url_message) {
