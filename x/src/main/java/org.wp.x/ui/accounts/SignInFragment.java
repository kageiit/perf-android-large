...
import org.wp.x.BuildConfig;
import org.wp.x.R;
import org.wp.x.WordPress;
import org.wp.x.models.Blog;
import org.wp.x.ui.accounts.helpers.FetchBlogListAbstract.Callback;
import org.wp.x.ui.accounts.helpers.FetchBlogListWPCom;
import org.wp.x.ui.accounts.helpers.FetchBlogListWPOrg;
import org.wp.x.ui.accounts.helpers.LoginAbstract;
import org.wp.x.ui.accounts.helpers.LoginWPCom;
...
import org.wp.x.util.HelpshiftHelper.Tag;
import org.wp.x.util.JSONUtils;
import org.wp.x.util.NetworkUtils;
import org.wp.x.util.StringUtils;
import org.wp.x.util.ToastUtils;
import org.wp.x.util.WPUrlUtils;
import org.wp.x.widgets.WPTextView;
...
//            track(Stat.LOGIN_FAILED, null);
    ...
    protected void showInvalidUsernameOrPasswordDialog() {
        ...
        nuxAlert = SignInDialogFragment.newInstance(getString(org.wp.x.R.string.nux_cannot_log_in),
                getString(org.wp.x.R.string.username_or_password_incorrect),
                org.wp.x.R.drawable.noticon_alert_big, 3, getString(
                        org.wp.x.R.string.cancel), getString(
                        org.wp.x.R.string.forgot_password), getString(
                        org.wp.x.R.string.contact_us), SignInDialogFragment.ACTION_OPEN_URL,
    ...
    protected void signInError(int messageId, String clientResponse) {
        FragmentTransaction ft = getFragmentManager().beginTransaction();
        SignInDialogFragment nuxAlert;
        if (messageId == org.wp.x.R.string.username_or_password_incorrect) {
            ...
            return;
        } else if (messageId == org.wp.x.R.string.invalid_site_url_message) {
