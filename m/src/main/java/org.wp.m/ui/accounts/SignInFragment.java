...
import org.wp.m.BuildConfig;
import org.wp.m.R;
import org.wp.m.WordPress;
import org.wp.m.models.Blog;
import org.wp.m.ui.accounts.helpers.FetchBlogListAbstract.Callback;
import org.wp.m.ui.accounts.helpers.FetchBlogListWPCom;
import org.wp.m.ui.accounts.helpers.FetchBlogListWPOrg;
import org.wp.m.ui.accounts.helpers.LoginAbstract;
import org.wp.m.ui.accounts.helpers.LoginWPCom;
...
import org.wp.m.util.HelpshiftHelper.Tag;
import org.wp.m.util.JSONUtils;
import org.wp.m.util.NetworkUtils;
import org.wp.m.util.StringUtils;
import org.wp.m.util.ToastUtils;
import org.wp.m.util.WPUrlUtils;
import org.wp.m.widgets.WPTextView;
...
//            track(Stat.LOGIN_FAILED, null);
    ...
    protected void showInvalidUsernameOrPasswordDialog() {
        ...
        nuxAlert = SignInDialogFragment.newInstance(getString(org.wp.m.R.string.nux_cannot_log_in),
                getString(org.wp.m.R.string.username_or_password_incorrect),
                org.wp.m.R.drawable.noticon_alert_big, 3, getString(
                        org.wp.m.R.string.cancel), getString(
                        org.wp.m.R.string.forgot_password), getString(
                        org.wp.m.R.string.contact_us), SignInDialogFragment.ACTION_OPEN_URL,
    ...
    protected void signInError(int messageId, String clientResponse) {
        FragmentTransaction ft = getFragmentManager().beginTransaction();
        SignInDialogFragment nuxAlert;
        if (messageId == org.wp.m.R.string.username_or_password_incorrect) {
            ...
            return;
        } else if (messageId == org.wp.m.R.string.invalid_site_url_message) {
