...
import org.wp.f.BuildConfig;
import org.wp.f.R;
import org.wp.f.WordPress;
import org.wp.f.models.Blog;
import org.wp.f.ui.accounts.helpers.FetchBlogListAbstract.Callback;
import org.wp.f.ui.accounts.helpers.FetchBlogListWPCom;
import org.wp.f.ui.accounts.helpers.FetchBlogListWPOrg;
import org.wp.f.ui.accounts.helpers.LoginAbstract;
import org.wp.f.ui.accounts.helpers.LoginWPCom;
...
import org.wp.f.util.HelpshiftHelper.Tag;
import org.wp.f.util.JSONUtils;
import org.wp.f.util.NetworkUtils;
import org.wp.f.util.StringUtils;
import org.wp.f.util.ToastUtils;
import org.wp.f.util.WPUrlUtils;
import org.wp.f.widgets.WPTextView;
...
//            track(Stat.LOGIN_FAILED, null);
    ...
    protected void showInvalidUsernameOrPasswordDialog() {
        ...
        nuxAlert = SignInDialogFragment.newInstance(getString(org.wp.f.R.string.nux_cannot_log_in),
                getString(org.wp.f.R.string.username_or_password_incorrect),
                org.wp.f.R.drawable.noticon_alert_big, 3, getString(
                        org.wp.f.R.string.cancel), getString(
                        org.wp.f.R.string.forgot_password), getString(
                        org.wp.f.R.string.contact_us), SignInDialogFragment.ACTION_OPEN_URL,
    ...
    protected void signInError(int messageId, String clientResponse) {
        FragmentTransaction ft = getFragmentManager().beginTransaction();
        SignInDialogFragment nuxAlert;
        if (messageId == org.wp.f.R.string.username_or_password_incorrect) {
            ...
            return;
        } else if (messageId == org.wp.f.R.string.invalid_site_url_message) {
