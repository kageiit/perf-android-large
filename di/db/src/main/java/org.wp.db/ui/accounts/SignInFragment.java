...
import org.wp.db.BuildConfig;
import org.wp.db.R;
import org.wp.db.WordPress;
import org.wp.db.models.Blog;
import org.wp.db.ui.accounts.helpers.FetchBlogListAbstract.Callback;
import org.wp.db.ui.accounts.helpers.FetchBlogListWPCom;
import org.wp.db.ui.accounts.helpers.FetchBlogListWPOrg;
import org.wp.db.ui.accounts.helpers.LoginAbstract;
import org.wp.db.ui.accounts.helpers.LoginWPCom;
...
import org.wp.db.util.JSONUtils;
import org.wp.db.util.NetworkUtils;
import org.wp.db.util.StringUtils;
import org.wp.db.util.ToastUtils;
import org.wp.db.util.WPUrlUtils;
import org.wp.db.widgets.WPTextView;
...
//            track(Stat.LOGIN_FAILED, null);
    ...
    protected void showInvalidUsernameOrPasswordDialog() {
        ...
        nuxAlert = SignInDialogFragment.newInstance(getString(org.wp.db.R.string.nux_cannot_log_in),
                getString(org.wp.db.R.string.username_or_password_incorrect),
                org.wp.db.R.drawable.noticon_alert_big, 3, getString(
                        org.wp.db.R.string.cancel), getString(
                        org.wp.db.R.string.forgot_password), getString(
                        org.wp.db.R.string.contact_us), SignInDialogFragment.ACTION_OPEN_URL,
    ...
    protected void signInError(int messageId, String clientResponse) {
        FragmentTransaction ft = getFragmentManager().beginTransaction();
        SignInDialogFragment nuxAlert;
        if (messageId == org.wp.db.R.string.username_or_password_incorrect) {
            ...
            return;
        } else if (messageId == org.wp.db.R.string.invalid_site_url_message) {
