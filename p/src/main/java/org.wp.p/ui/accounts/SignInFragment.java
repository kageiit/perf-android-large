...
import org.wp.p.BuildConfig;
import org.wp.p.R;
import org.wp.p.WordPress;
import org.wp.p.models.Blog;
import org.wp.p.ui.accounts.helpers.FetchBlogListAbstract.Callback;
import org.wp.p.ui.accounts.helpers.FetchBlogListWPCom;
import org.wp.p.ui.accounts.helpers.FetchBlogListWPOrg;
import org.wp.p.ui.accounts.helpers.LoginAbstract;
import org.wp.p.ui.accounts.helpers.LoginWPCom;
...
import org.wp.p.util.HelpshiftHelper.Tag;
import org.wp.p.util.JSONUtils;
import org.wp.p.util.NetworkUtils;
import org.wp.p.util.StringUtils;
import org.wp.p.util.ToastUtils;
import org.wp.p.util.WPUrlUtils;
import org.wp.p.widgets.WPTextView;
...
//            track(Stat.LOGIN_FAILED, null);
    ...
    protected void showInvalidUsernameOrPasswordDialog() {
        ...
        nuxAlert = SignInDialogFragment.newInstance(getString(org.wp.p.R.string.nux_cannot_log_in),
                getString(org.wp.p.R.string.username_or_password_incorrect),
                org.wp.p.R.drawable.noticon_alert_big, 3, getString(
                        org.wp.p.R.string.cancel), getString(
                        org.wp.p.R.string.forgot_password), getString(
                        org.wp.p.R.string.contact_us), SignInDialogFragment.ACTION_OPEN_URL,
    ...
    protected void signInError(int messageId, String clientResponse) {
        FragmentTransaction ft = getFragmentManager().beginTransaction();
        SignInDialogFragment nuxAlert;
        if (messageId == org.wp.p.R.string.username_or_password_incorrect) {
            ...
            return;
        } else if (messageId == org.wp.p.R.string.invalid_site_url_message) {
